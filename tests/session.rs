...
fn session_create_test() {
    ...
    let (created_session_entry, _) = client
        .create(&entry, None)
        .unwrap();
...
fn session_info_test() {
    ...
    let created_session_entry_id = created_session_entry
        .ID
        .unwrap();

    let (session_entries, _) = client
        .info(&created_session_entry_id, None)
        .unwrap();
...
fn get_number_of_session_entries_with_matching_name(client: &Client, unique_test_identifier: &str) -> usize {
    let (session_entries, _) = client
        .list(None)
        .unwrap();
