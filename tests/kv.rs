...
fn kv_add_test() {
    ...
    assert_eq!(kv_list_result.0.len(), 3);
    ...
    client.put(&new_kv_pair, None).unwrap();

    let kv_list_result = client.list(&unique_test_path, None).unwrap();
    assert_eq!(kv_list_result.0.len(), 4);
...
fn kv_delete_test() {
    ...
    let kv_list_result = client.list(&unique_test_path, None).unwrap();
    assert_eq!(kv_list_result.0.len(), 2);

    let actual_key_names = kv_list_result
        .0
        .iter()
        .map(|kv| kv.Key.split('/').nth(1).unwrap())
        .collect::<Vec<&str>>();
...
fn kv_get_test() {
    ...
    assert_eq!(kv_pair.0.unwrap().Value, "\"secondvalue\"");
...
fn kv_test() {
    ...
    // test list
    let r = client.list(&unique_test_path, None).unwrap();
    assert!(!r.0.is_empty());
    ...
    let kv_pair = client.get(&key_to_get, None).unwrap();
    assert!(kv_pair.0.is_some());
    assert_eq!(kv_pair.0.unwrap().Value, "\"updatedsecondvalue\"");
...
fn set_up() -> (Client, String) {
    ...
    for kv_pair in kv_pairs {
        client.put(&kv_pair, None).unwrap();
    }

    (client, unique_test_path)
}
